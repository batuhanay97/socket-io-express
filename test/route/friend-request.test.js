...
describe('FRIEND REQUEST ROUTE', () => {
    ...
    let routePrefix = 'api/v1/friend-request';

    // Read each test file for v1
    fs
        .readdirSync(__dirname + '/../controller/v1/friend-request')
        .forEach(file => {
            if (filterTests.length === 0 || filterTests.includes(file))
                require(__dirname + '/../controller/v1/friend-request/' + file)(app, routePrefix);
        });
